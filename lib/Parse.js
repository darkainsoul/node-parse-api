...
Parse.prototype = {
    ...
    _api_host: 'parseapi.back4app.com',
    _api_port: 443,

    // add object to class store
    insert: function (className, object, callback) {
        parseRequest.call(this, 'POST', '/classes/' + className, object, callback);
    },

    // add files
    insertFile: function (fileName, data, contentType, callback) {
        parseRequest.call(this, 'POST', '/files/' + fileName, data, callback, contentType);
    },

    // create a new user
    insertUser: function (data, callback) {
        parseRequest.call(this, 'POST', '/users/', data, callback);
    },

    findWithObjectId: function (className, objectId, callback) {
        var url = '/' + (className === '_User' ? 'users' : 'classes/' + className + '/' + objectId);
    ...
    find: function (className, query, callback) {
        var url = '/' + (className === '_User' ? 'users' : 'classes/' + className);
    ...
    // get an object from the class store with acl permissions
    findMyRecords: function (className, query, sessionToken, callback) {
        var url = '/' + (className === '_User' ? 'users' : 'classes/' + className);
        var queryType = typeof query;
        if (queryType === 'string') {
            url += '/' + query;
    ...
    // Call a function in the cloud
    findCloudFunction: function (functionName, data, callback) {
        parseRequest.call(this, 'POST', '/functions/' + functionName, data, callback);
    },
    ...
    batch: function (requests, callback) {
        parseRequest.call(this, 'POST', '/batch', {requests: requests}, callback);
    },

    // user login
    loginUser: function (username, password, callback) {
        parseRequest.call(this, 'GET', '/login/?username=' + username + '&password=' + password, null, callback);
    },

    // user logout
    logout: function (objectId, sessionToken, callback) {
        parseRequest.call(this, 'DELETE', '/sessions' + '/' + objectId, null, callback, null, sessionToken);
    },

    // user logout
    logoutUser: function (sessionToken, callback) {
        parseRequest.call(this, 'POST', '/logout', null, callback, null, sessionToken);
    },

    // user sessions
    sessions: function (sessionToken, callback) {
        parseRequest.call(this, 'GET', '/sessions', null, callback, null, sessionToken);
    },

    // retrieve current user
    me: function (sessionToken, callback) {
        parseRequest.call(this, 'GET', '/users/me', null, callback, null, sessionToken);
    },
    ...
    findFileByUser: function (userId, className, callback) {
        ...
        parseRequest.call(this, 'GET', '/classes/' + className + '?' + encodedString, null, callback);
    },
    ...
    insertCustom: function (className, object, callback) {
        parseRequest.call(this, 'POST', '/' + className, object, callback);
    },

    // update an object in the class store
    update: function (className, objectId, object, callback) {
        parseRequest.call(this, 'PUT', '/classes/' + className + '/' + objectId, object, callback);
    },
    ...
    updateUserEmail: function (objectId, data, sessionToken, callback) {
        ...
        data = {email: data};
        parseRequest.call(this, 'PUT', '/users/' + objectId, data, callback, null, sessionToken);
    },
    ...
    passwordReset: function (data, callback) {
        data = {email: data};
        parseRequest.call(this, 'POST', '/requestPasswordReset', data, callback);
    },

    // remove an object from the class store
    delete: function (className, objectId, callback) {
        parseRequest.call(this, 'DELETE', '/classes/' + className + '/' + objectId, null, callback);
    },
    ...
    deleteUser: function (objectId, sessionToken, callback) {
        ...
        parseRequest.call(this, 'DELETE', '/users/' + objectId, null, callback, null, sessionToken);
    },
    ...
    insertInstallationData: function (deviceType, deviceToken, callback) {
        ...
        parseRequest.call(this, 'POST', '/installations/', data, callback);
    },
    ...
    updateInstallationDataChannels: function (objectId, channels, callback) {
        parseRequest.call(this, 'PUT', '/installations/' + objectId, {
            channels: channels
        }, callback);
    },

    getInstallationData: function (callback) {
        parseRequest.call(this, 'GET', '/installations', null, callback);
    },

    getInstallationDataForDeviceToken: function (deviceToken, callback) {
        parseRequest.call(this, 'GET', '/installations?where={"deviceToken":"' + deviceToken + '"}', null, callback);
    },

    deleteInstallation: function (objectId, callback) {
        parseRequest.call(this, 'DELETE', '/installations/' + objectId, null, callback);
    },
    ...
    countObjects: function (className, query, callback) {
        if (typeof(query) === "function") {
            parseRequest.call(this, 'GET', '/classes/' + className, null, query);
        }
        if (typeof(query) === "string") {
            parseRequest.call(this, 'GET', '/classes/' + className + '/' + query, null, callback);
        } else {
            parseRequest.call(this, 'GET', '/classes/' + className, {where: JSON.stringify(query)}, callback);
        }
    },
    ...
    addRelation: function (relationName, className1, objectId1, className2, objectId2, callback) {
        ...
        data[relationName] = {
            __op: "AddRelation",
            objects: [{__type: "Pointer", className: className2, objectId: objectId2}]
        };
        parseRequest.call(this, 'PUT', '/classes/' + className1 + '/' + objectId1, data, callback);
    },
    ...
    insertRole: function (data, callback) {
        parseRequest.call(this, 'POST', '/roles/', data, callback);
    },

    getRole: function (objectId, callback) {
        parseRequest.call(this, 'GET', '/roles/' + objectId, null, callback);
    },

    getRoles: function (params, callback) {
        if (typeof(params) === "function") {
            parseRequest.call(this, 'GET', '/roles/', null, params);
        }
        if (typeof(params) === "string") {
            parseRequest.call(this, 'GET', '/roles/' + params, null, callback);
        } else {
            params = JSON.stringify(params);
            parseRequest.call(this, 'GET', '/roles/', params, callback);
        }
    },

    updateRole: function (objectId, data, callback) {
        parseRequest.call(this, 'PUT', '/roles/' + objectId, data, callback);
    },

    deleteRole: function (objectId, callback) {
        parseRequest.call(this, 'DELETE', '/roles/' + objectId, callback);
    },

    sendPush: function (data, callback) {
        parseRequest.call(this, 'POST', '/push/', data, callback);
    },

    getCurrentSession: function (objectId, callback) {
        parseRequest.call(this, 'GET', '/sessions/' + objectId, callback);
    }
...
function toDeleteOps(className, objects) {
    return objects.map(function (object) {
        return {
            method: 'DELETE',
            path: '/' + ( className === '_User' ? 'users/' : 'classes/' + className + '/' ) + object.objectId
        };
    });
}
